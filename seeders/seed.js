...
async function seed() {
  try {
    ...
    const [user] = await User.findOrCreate({
      ...
      defaults: { email: 'rizki@example.com', password: hashedPassword },
    });
    ...
    if (existingProducts === 0) {
      await Product.bulkCreate([
        ...
        { name: 'Sepatu Sneakers Canvas', description: 'Cocok buat kasual', price: 220000, stock: 30 },
        {
          ...
          description: 'Produk ini sengaja dibikin buat demo stored XSS, liat README',
          price: 50000,
          stock: 5,
        },
      ]);
      console.log('Produk dummy berhasil ditambahin (termasuk 1 produk demo XSS)');
    } else {
      console.log('Produk udah ada, skip supaya gak dobel');
    }

    console.log('\nSeeding selesai ✅');
    console.log('Login pake: username=rizki password=password123');
    process.exit(0);
  } catch (err) {
